...
{
    cascadeCNN_ = new CascadeCNN(model_file, trained_file, mean_file);
...
{
    return file_list_read(dataset_path_ + "FDDB-folds/file_list.txt");
}
...
{
    return img_path_read(dataset_path_ + "FDDB-folds/" + file_list_[i]);
}
...
{
    string write_path = img_path_convert(dataset_path_ + "FDDB-folds/" + file_list_[i_]);
    std::fstream file(write_path + "-CascadeCNN.txt", ios::app);
...
bool fddb_detection::img_write(std::vector<cv::Rect> rects, std::vector<float> confidence, std::string img_path)
{
    ...
    cv::imwrite(dataset_path_ + "CascadeCNN/" + img_path + "-CascadeCNN.jpg", img);
